...
type SortField = "nama_user" | "email" | "role";
...
export default function PenggunaPageClient({
  ...
  async function handleSubmit(data: {
    ...
    const result = modal.editId
      ? await updatePenggunaAction(modal.editId, data)
      : await createPenggunaAction(data);
    ...
    showSuccess(
      modal.isEdit ? "Pengguna berhasil diperbarui" : "Pengguna berhasil ditambahkan"
    );
    modal.close();
    startTransition(() => {});
  }
  ...
  async function handleDelete() {
    if (!deleteId) return;
    const result = await deletePenggunaAction(deleteId);
    if (!result.ok) {
      showError(result.message);
      return;
    }
  ...
  return (
    <>
      <div className="space-y-6">
        ...
        <FilterBar
          onReset={() => resetParams(["search", "sort", "role"])}
        >
          <FilterSearch
            value={search}
            onChange={setSearch}
      ...
      <FormDialog
        ...
        title={modal.isEdit ? "Edit Pengguna" : "Tambah Pengguna"}
        description={
          modal.isEdit
